...
export default {
  ...
  computed: {
    uZIndex() {
      return (this.zIndex ? this.zIndex : 970) as ZIndexProperty;
    },
  },
  mounted() {
    this.initObserver();
  ...
  methods: {
    ...
    observeContent() {
      this.disconnectObserver('contentObserver');
      const contentObserver = (this as any).createIntersectionObserver({ thresholds: [0.95, 0.98, 1] });
      contentObserver.relativeToViewport({ top: -this.stickyTop });
      contentObserver.observe(
        `.${this.elClass}`,
        (res: { boundingClientRect: { top: any } }) => {
          if (!this.enable) return;
          this.setFixed(res.boundingClientRect.top);
        }
      );
      (this as any).contentObserver = contentObserver;
    },
    setFixed(top: number) {
    ...
    disconnectObserver(observerName: string) {
      const observer = (this as any)[observerName];
