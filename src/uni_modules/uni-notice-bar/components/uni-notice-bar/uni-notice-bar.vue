...
export default {
  ...
  data() {
    const elId = `Uni_${Math.ceil(Math.random() * 10e5)
      .toString(36)}`;
    const elIdBox = `Uni_${Math.ceil(Math.random() * 10e5)
      .toString(36)}`;
    return {
      textWidth: 0,
      boxWidth: 0,
      ...
      if (this.scrollable) {
        ...
        dom.getComponentRect(this.$refs['animationEle'], res => {
          ...
          animation.transition(
            ...
            {
              styles: { transform: `translateX(-${winWidth}px)` },
              duration: 0,
            ...
            () => {
              if (!this.stopAnimation) {
                animation.transition(
                  this.$refs['animationEle'],
                  {
                    styles: { transform: `translateX(-${this.textWidth}px)` },
                    timingFunction: 'linear',
